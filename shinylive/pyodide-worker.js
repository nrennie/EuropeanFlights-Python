...
var loadBinaryFile;
var loadScript;
if (loadBinaryFile = IN_NODE ? async function(indexURL, path, _file_sub_resource_hash) {
  if (path.startsWith("/") || path.includes("://") || (path = `${indexURL}${path}`), path.startsWith("file://") && (path = path.slice("file://".length)), path.includes("://")) {
  ...
  const base = new URL(indexURL, location), url = new URL(path, base);
...
function finalizeBootstrap(API, config) {
  ...
  return importhook.register_js_module("pyodide_js", pyodide2), API.pyodide_py = import_module("pyodide"), API.pyodide_code = import_module("pyodide.code"), API.pyodide_ffi = import_module("pyodide.ffi"), API.package_loader = import_module("pyodide._package_loader"), API.version = API.pyodide_py.__version__, pyodide2.pyodide_py = API.pyodide_py, pyodide2.version = API.version, pyodide2.globals = API.globals, pyodide2;
}
async function loadPyodide(options = {}) {
  options.indexURL || (options.indexURL = function() {
    let err;
    try {
      throw new Error();
    } catch (e) {
  ...
  const default_config = { fullStdLib: true, jsglobals: globalThis, stdin: globalThis.prompt ? globalThis.prompt : void 0, homedir: "/home/pyodide", lockFileURL: options.indexURL + "repodata.json" }, config = Object.assign(default_config, options);
  await async function() {
    if (!IN_NODE)
      return;
    if (nodeUrlMod = (await import("url")).default, nodeFsPromisesMod = await import("fs/promises"), nodeFetch = globalThis.fetch ? fetch : (await import("node-fetch")).default, nodeVmMod = (await import("vm")).default, "undefined" != typeof __require)
      return;
  ...
  const pyodide_py_tar_promise = loadBinaryFile(config.indexURL, "pyodide_py.tar"), Module = { noImageDecoding: true, noAudioDecoding: true, noWasmDecoding: false, preloadedWasm: {}, preRun: [] }, API = { config };
  ...
  await loadScript(scriptSrc), await _createPyodideModule(Module), await moduleLoaded, Module.locateFile = (path) => {
  ...
  if (pyodide2.version.includes("dev") || API.setCdnUrl(`https://cdn.jsdelivr.net/pyodide/v${pyodide2.version}/full/`), await API.packageIndexReady, API.repodata_info.version !== pyodide2.version)
